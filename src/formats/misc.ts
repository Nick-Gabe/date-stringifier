...
const miscReplacer: DateReplacer = {
  ...
  rel: (date, lang) => {
    ...
    const elapsedSeconds = Math.abs(elapsed),
      elapsedMinutes = Math.abs(Math.floor(elapsed / 60)),
      elapsedHours = Math.abs(Math.floor(elapsed / 3600)),
      elapsedDays = Math.abs(Math.floor(elapsed / 86400))
    ...
    if(elapsed <= 0) {
      ...
      // Less than an hour
        ...
        return `${fromNow} ${elapsedMinutes} ${minute+plural(elapsedMinutes)}`
      }
      // Less than a day
