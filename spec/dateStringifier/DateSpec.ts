...
describe('The function parameters', function () {
  ...
  it('should not accept invalid dates', function () {
    expect(() => format(new Date('example'))).toThrowError(
      'Invalid Date is not a valid Date',
    );
    expect(() => format('Not a date' as unknown as Date)).toThrowError(
  ...
  it('should not accept invalid languages only if any date needs it', function () {
    ...
    expect(() =>
      format(date, '{oDD}', 'invalid' as AvailableLanguages),
    ).toThrowError('invalid is not a valid language');
  });
});
...
describe('The returned Date', function () {
  ...
  const baseDate = new Date(
    new Date(1656188882000).toLocaleString('en-US', {
  ...
  it('must be correct', function () {
    ...
    expect(format(baseDate, '{day} {month} {hour} {minute} {second}')).toBe(
      'Saturday June Seventeen Twenty-eight Two',
    );
  ...
  it('must change depending on language', function () {
    expect(format(baseDate, '{oDD} dia de {month}', 'pt')).toBe(
      '25º dia de Junho',
    );
    expect(format(baseDate, 'Hoje é {day}', 'pt')).toBe('Hoje é Sábado');
    expect(
      format(baseDate, 'Agora são {hour} horas, {hour12} da tarde', 'pt'),
    ).toBe('Agora são Dezessete horas, Cinco da tarde');
    expect(format(baseDate, '{minute} minutos e {second} segundos', 'pt')).toBe(
